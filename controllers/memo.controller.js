...
module.exports.getAllMemo = async (req, res) => {
    console.log("zo");
    Memo.find({ userId: convertToObjectId(req.userData._id) })
        .populate("category")
        .exec()
        .then((doc) => {
            res.send(doc);
...
module.exports.getOneMemo = async (req, res) => {
    ...
    let data = await Memo.findOne({ _id: id })
        .exec()
        .catch((err) => []);
    res.send(data);
};
...
module.exports.getMemoGroupByIDCategory = async (req, res) => {
    let id = req.params.id;
    let data = await Memo.findOne({ category: id })
        .exec()
        .catch((err) => []);
    res.send(data);
};
...
module.exports.addNewMemo = async (req, res) => {
    ...
    memo.save()
        .then((a) => {
            res.send({ ...a._doc, category: data.category });
        })
...
module.exports.deleteMemo = async (req, res) => {
    let data = req.body;
    Memo.deleteOne({ _id: convertToObjectId(data._id) })
        .exec()
        .then((a) => res.send("ok"))
